...
% This function computes the significance for spatial association using ESSE
% Inputs:
...
%   xparc: either [], 'auto', or int vector of length N where each value represents a parcellation.
    ...
    %   sESSE). if 'auto', run nESSE by determining the parcel using a
...
%   crossblock: ways to infer between-parcel covariance. Although 'conv'
    ...
    %   is the default. Only work in nESSE, i.e., number of parcels are > 1.
...
%   M_scale: the scaling factor to determine the number of lag distances
    ...
    %   settings.
% Outputs:
%   pef: statistical significance p-value derived using ESSE
%   rX: pearson correlation coefficient
%   nef: effective sample size estiamted with ESSE
%   run_status: bool indicator whether ESSE estimation is valid, 1
...
% now run the stationary stable model without parcellation, i.e., sESSE
...
% estimate the covariance matrix by parcellation if n_parc > 1, i.e., nESSE
if xn_parc > 1
